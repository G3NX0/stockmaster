...
@section('breadcrumb')
    ...
    <i data-lucide="chevron-right" class="w-3 h-3 text-onyx-300"></i>
    <span class="text-onyx-950 dark:text-white uppercase">{{ $unit->symbol }}</span>
@endsection

@section('content')
<div class="max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="mb-8">
        <h1 class="text-3xl font-black tracking-tighter italic leading-none uppercase">Modify Measurement</h1>
    ...
    <div class="zen-glass squircle overflow-hidden">
        <form action="{{ route('units.update', $unit->id) }}" method="post" class="p-10 space-y-8">
            @csrf
            @method('PUT')
