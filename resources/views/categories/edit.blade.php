...
@section('breadcrumb')
    ...
    <i data-lucide="chevron-right" class="w-3 h-3 text-onyx-300"></i>
    <span class="text-onyx-950 dark:text-white uppercase">{{ $category->name }}</span>
@endsection

@section('content')
<div class="max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="mb-8">
        <h1 class="text-3xl font-black tracking-tighter italic leading-none uppercase">Modify Category</h1>
        <p class="text-[10px] font-black uppercase tracking-[0.3em] text-onyx-400 mt-2">Protocol: Record Update</p>
    </div>

    <div class="zen-glass squircle overflow-hidden">
        <form action="{{ route('categories.update', $category->id) }}" method="post" class="p-10 space-y-8">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div class="space-y-3">
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" id="description" rows="3"
                        ...
                        placeholder="Brief summary of category scope...">{{ old('description', $category->description) }}</textarea>
                </div>
            </div>
