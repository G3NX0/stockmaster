...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Edit Data Barang</h1>
    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Perbarui informasi barang yang sudah ada di inventaris.</p>
</div>

<div class="max-w-2xl">
    <div class="glass rounded-[2rem] shadow-2xl border border-white/40 dark:border-white/5 overflow-hidden">
        <form action="{{ route('items.update', $item->id) }}" method="post" class="p-10 space-y-8">
            @csrf
            @method('PUT')

            @if ($errors->any())
            <div class="p-6 rounded-2xl bg-rose-500/10 border border-rose-500/20 mb-8 animate-in fade-in zoom-in duration-500">
                <div class="flex items-center gap-3 text-rose-500 mb-3">
                    <i data-lucide="alert-circle" class="w-5 h-5"></i>
                    <span class="text-xs font-black uppercase tracking-widest">Input Errors Detected</span>
                </div>
                <ul class="space-y-1">
                    @foreach ($errors->all() as $error)
                        <li class="text-[11px] text-rose-400 font-medium list-disc ml-4">{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
            @endif
            ...
            <div class="grid grid-cols-1 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative group">
                            ...
                            <input type="number" name="stok_barang" id="stok_barang" required
                                class="block w-full pl-12 pr-5 py-4 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-white/5 rounded-2xl text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                                value="{{ old('stok_barang', $item->stok_barang) }}">
                        </div>
                    </div>

                    <div class="space-y-3">
                        <label for="min_stock" class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Minimum Stock (Alert)</label>
                        <div class="relative group">
                            ...
                            <input type="number" name="min_stock" id="min_stock" required
                                class="block w-full pl-12 pr-5 py-4 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-white/5 rounded-2xl text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all"
                                value="{{ old('min_stock', $item->min_stock) }}">
                        </div>
                    </div>
                </div>

                <div class="p-6 rounded-3xl bg-slate-100/50 dark:bg-white/5 border border-slate-200 dark:border-white/5" x-data="{ isAsset: {{ $item->is_asset ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] text-slate-400 font-medium">Enable depreciation tracking</span>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_asset" value="1" class="sr-only peer" x-model="isAsset" {{ $item->is_asset ? 'checked' : '' }}>
                    ...
                    <div x-show="isAsset" x-transition class="space-y-6 pt-4 border-t border-slate-200 dark:border-white/10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="date" name="purchase_date" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm" value="{{ $item->purchase_date ? $item->purchase_date->format('Y-m-d') : '' }}">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Useful Life (Months)</label>
                                <input type="number" name="useful_life_months" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm" value="{{ $item->useful_life_months }}" placeholder="e.g. 24">
                            </div>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Salvage Value (Rp)</label>
                            <input type="number" name="salvage_value" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm" value="{{ $item->salvage_value }}" placeholder="Residual value">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <div class="relative" x-data="{ open: false, selectedId: '{{ $item->category_id }}', selectedName: '{{ $item->category?->name ?? 'Pilih Kategori' }}' }">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative" x-data="{ open: false, selectedId: '{{ $item->unit_id }}', selectedName: '{{ $item->unit?->name ?? 'Pilih Satuan' }}' }">
